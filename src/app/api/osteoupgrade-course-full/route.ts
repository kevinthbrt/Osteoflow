...
const PROXY_URL = 'https://osteoupgrade.vercel.app/api/osteoflow/course-full'
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!formationId || !email) {
      return NextResponse.json({ error: 'formation_id et email requis' }, { status: 400, headers: noStore })
    }
    ...
    try {
      ...
      return NextResponse.json({ error: 'Timeout' }, { status: 503, headers: noStore })
    }

    if (!proxyRes.ok) {
      const body = await proxyRes.json().catch(() => ({}))
      return NextResponse.json(body, { status: proxyRes.status, headers: noStore })
    }
