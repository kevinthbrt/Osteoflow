...
export async function GET() {
  try {
    ...
    console.error('[API/emails/settings] GET error:', message)
...
export async function POST(request: Request) {
  try {
    const { createClient } = await import('@/lib/db/server')
    const { testSmtpConnection } = await import('@/lib/email/smtp-service')
    const { testImapConnection } = await import('@/lib/email/imap-service')
    const { emailSettingsSchema } = await import('@/lib/validations/settings')
    ...
    const message = error instanceof Error ? error.message : 'Unknown error'
    console.error('[API/emails/settings] POST error:', message)
    return NextResponse.json({ error: message }, { status: 500 })
  }
}
...
export async function DELETE() {
  try {
    ...
    const message = error instanceof Error ? error.message : 'Unknown error'
    console.error('[API/emails/settings] DELETE error:', message)
    return NextResponse.json({ error: message }, { status: 500 })
  }
}
