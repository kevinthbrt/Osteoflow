...
export async function GET() {
  try {
    ...
    return NextResponse.json({ configured: !!key, preview: key ? `sk-ant-...${key.slice(-4)}` : null })
  } catch {
...
export async function POST(req: Request) {
  try {
    ...
    if (key) {
      db.prepare(
        "INSERT INTO app_config (key, value) VALUES ('anthropic_api_key', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value"
      ).run(key.trim())
    } else {
